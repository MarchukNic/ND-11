...
app.post("/api/usersSearch", jsonParser, function (req, res) {
    ...
    mongoClient.connect(url, function (err, db) {
        if (userLastName != '') {
            db.db('usersdb').collection("users").find({ lastname: userLastName }).toArray(function (err, users) {
                res.send(users)
                db.close();
            });
        } else  if (userName != '') {
            db.db('usersdb').collection("users").find({ name: userName }).toArray(function (err, users) {
                res.send(users)
                db.close();
            });
        } else if (userPhone != '') {
            db.db('usersdb').collection("users").find({ phone: userPhone }).toArray(function (err, users) {
                res.send(users)
                db.close();
            });
        }
